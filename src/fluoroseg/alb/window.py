...
from .utils import SampleArg, sample
...
def adjustable_window(
    ...
    """Apply an adjustable window to intensity images.
    ...
    def _adjustable_window(images: np.ndarray, **kwargs) -> np.ndarray:
        return adjustable_window_fn(images, x_lo, x_hi, y_lo, y_hi, quantile)

    def f_id(x, **kwargs):
        return x

    return A.Lambda(
        image=_adjustable_window,
        mask=f_id,
        keypoints=f_id,
        bboxes=f_id,
        name="adjustable_window",
    )
